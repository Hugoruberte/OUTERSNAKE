...
public class SnakeController : MonoBehaviour 
{
	...
	public void StartSnakeMovement()
	{
		...
		this.moveCoroutine = this.UpdateSnakeMovementCoroutine();
	...
	public void StopSnakeMovement()
	{
		...
		this.StopAllCoroutines();
		this.moveCoroutine = this.StopSnakeMovementCoroutine();
		this.StartCoroutine(this.moveCoroutine);
	}
	...
	private IEnumerator UpdateSnakeMovementCoroutine()
	{
		while(true)
		{
			...
			if(this.faceswitcher.IsSwitchingFace() == false) {
				// Search for possible new faces
				this.SearchForNewFace(this.direction);
			}

			// If found a new face :
			if(this.faceswitcher.IsSwitchingFace() == true) {
			...
			if(Vector3.Distance(this.myRigidbody.position, this.targetPosition) > this.data.accuracy) {
				...
				this.data.snakeMovementEvents.onStartStepTo?.Invoke(this.targetPosition, this.myRigidbody.rotation * Vector3.up);
	...
	private void ApplyInputsAndRotateSnake()
	{
		...
		if(!this.faceswitcher.IsSwitchingFace())
		{
	...
	private Quaternion HeartRotationOverFace(Vector3 dir)
	{
		...
		axis = Vector3Int.RoundToInt(this.myRigidbody.rotation * Vector3.right);
	...
	private struct FaceSwitcher
	{
		...
		public bool IsSwitchingFace()
		{
			if (this.switching)
			{
				if (this.delay > 0) { this.delay--; }
				if (this.delay == 0) { this.switching = false; }
			}
