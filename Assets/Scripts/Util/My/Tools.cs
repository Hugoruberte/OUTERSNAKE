...
namespace My.Tools
{
	...
	{
		public static float GetRealDuration(this ParticleSystem particle)
		{
			if(particle.main.loop) {
				Debug.LogWarning("WARNING : This particle system is looping and thus it doesn't have a 'real duration' !");
				return -1f;
			}
			...
			do {
				particle.Simulate(current);
				current += Time.deltaTime;
			}
			while(particle.IsAlive(true) && current < 999f);

			if(current >= 999f) {
				Debug.LogWarning("WARNING : This particle system 'real duration' could not be calculated ! It is too long !");
			}
