...
public abstract class CellableMovementController : MovementController
{
	...
	protected bool TargetIsTooFarAway(Cell c, int maxStepDistance)
	{
		Vector3 relative;
	...
	protected Cell GetOneCellWithDirection(Vector3 dir, int maxStepDistance)
	{
		Grill g;
		Cell res;
		int count;
		...
		do {
			res = g.GetOneCellInThisDirection(this.entity.cellable.currentCell, dir, maxStepDistance);
			count --;
		}
		while((res == null || !res.isWalkable || !this.PathIsFreeTo(res)) && count > 0);
