...
public class WhiteRabbitAI : UtilityBehaviourAI
{
	...
	void Start()
	{
		snake = SnakeManager.GetSnake().transform;
	...
	public float DistanceToDanger(MovementController ctr)
	{
		...
		foreach(IDangerousEntity d in this.dangers) {
			dist = Vector3.Distance(d._transform.position, ctr.myTransform.position);
	...
	public IEnumerator RunAway(MovementController ctr, UtilityAction act)
	{
		...
		yield return ctr.StepToRunAway(from);
